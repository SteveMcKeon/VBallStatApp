...
const MainPage = () => {
  ...
  const [displayNamesById, setDisplayNamesById] = useState({});
  ...
  const loadStatsForSelectedVideo = async (videoUrl) => {
    ...
    if (statError || !statData) {
      ...
      const userIds = Array.from(
        new Set(
          statData.flatMap(s => [
            s.player_user_id ? String(s.player_user_id) : null,
            s.set_to_user_id ? String(s.set_to_user_id) : null,
          ]).filter(Boolean)
        )
      );
      if (userIds.length) {
        try {
          ...
          if (resp.ok && json?.names) {
            setDisplayNamesById(json.names);
          } else {
            console.warn('display-names failed', json);
            setDisplayNamesById({});
          }
        } catch (e) {
          console.warn('display-names error', e);
          setDisplayNamesById({});
        }
      }
    }
  ...
  useEffect(() => {
    const onDisplayNameUpdated = (e) => {
      const { id, name } = e.detail || {};
      if (!id || typeof name !== 'string') return;
      setDisplayNamesById(prev => ({ ...prev, [String(id)]: name }));
    };
    window.addEventListener('display_names_updated', onDisplayNameUpdated);
    return () => window.removeEventListener('display_names_updated', onDisplayNameUpdated);
  }, []);
  useEffect(() => {
        ...
        >
          <div className="h-full flex flex-col">
            <div className="w-full">
              ...
              <div className={`flex items-center p-2 ${isStats ? 'justify-between' : 'justify-end'}`}>
                {isStats && (
